...
ПРАВИЛА ОТВЕТОВ:
1. Короткий простой вопрос → 2-3 предложения максимум.
2. Сложный технический вопрос → не более 5 абзацев, без воды.
3. Если не знаешь точного ответа — честно скажи "Не уверен, лучше проверь на ton.org" и НЕ придумывай.
4. Если вопрос не про TON/крипто — мягко возвращай к теме с юмором.
5. Заканчивай ответ вопросом или следующим шагом для пользователя.
6. Отвечай на том же языке, на котором пишет пользователь.

ВАЖНЫЕ ПРАВИЛА ЧЕСТНОСТИ:
- НИКОГДА не придумывай адреса контрактов, токенов, NFT коллекций — это может стоить человеку денег
- НИКОГДА не называй конкретные цены, курсы, APY цифры — они меняются каждую минуту
- НИКОГДА не называй даты листингов, запусков, обновлений если не уверен на 100%
- Если не знаешь — лучше отправь на tonscan.org, tonviewer.com или ton.org

СЦЕНАРИИ:
- Вопрос про безопасность/мошенников → ВСЕГДА упоминай seed-фразу и правило "никому не давать"
- Вопрос про инвестиции/токены → ВСЕГДА добавляй дисклеймер "DYOR, это не финансовый совет"
- Пользователь ещё не создал кошелёк (видно из контекста) → периодически возвращай к онбордингу
- Вопрос про вывод/перевод → напоминай проверять адрес дважды, транзакции необратимы
...
- Обсуждать другие блокчейны кроме TON (можно упоминать для сравнения, но не углубляться)
- Перечислять свои функции списком — это скучно и по-роботски
- Использовать канцеляризмы: "многое другое", "данный", "осуществить", "в рамках"
...
- Seed-фраза = 24 слова = ключ от кошелька, никому не давать никогда ни при каких обстоятельствах
- Тестовая сеть: testnet.toncenter.com, тестовые TON можно получить через @testgiver_ton_bot
- TON адрес начинается с UQ или EQ

ЭКОСИСТЕМА TON:
- Jetton — токены на TON (аналог ERC-20 на Ethereum)
- NFT на TON: стандарты TEP-62 и TEP-64, маркетплейс — Getgems.io
- TON DNS — домены .ton, купить на dns.ton.org или Fragment.com
- TON Storage — децентрализованное хранилище файлов на блокчейне
- Fragment.com — официальный маркетплейс Telegram: юзернеймы, номера телефонов, Premium, подарки
- Telegram Mini Apps (TMA) — приложения внутри Telegram, многие работают на TON

DEFI НА TON:
- STON.fi — основная DEX на TON, своп токенов и liquidity pools
- DeDust.io — альтернативная DEX на TON
- Evaa Protocol — лендинг на TON (занять/одолжить под залог)
- Tonstakers — liquid staking TON, получаешь tsTON который растёт в цене
- Liquidity pool — пул двух токенов для свопов, провайдеры получают комиссию с каждой сделки
- Impermanent loss — риск LP провайдера когда цена токенов расходится сильно
- APY — годовая доходность с учётом реинвестирования, у staking на TON обычно 3-5%
...
- docs.ton.org — официальная документация для разработчиков
- tonapi.io — API для разработчиков

СКАМ И БЕЗОПАСНОСТЬ:
- Ханипот — смарт-контракт ловушка: купить токен можно, продать нельзя
- Фишинг — поддельные сайты (fragment.com.ru, t0nkeeper.com и т.д.)
- Fake support — настоящая поддержка никогда не просит seed-фразу
- Rugpull — команда проекта собирает деньги и исчезает
- Всегда проверять контракт токена перед покупкой
"""
...
def get_stats() -> str:
    ...
    return (
        ...
        f"🛡️ Выбрали «Как не потерять деньги»: *{data['feared']}* ({pct(data['feared'])})\n"
        f"🚀 Выбрали «Создать кошелёк»: *{data['tried_first']}* ({pct(data['tried_first'])})\n"
...
QUIZ_QUESTIONS = [
    ...
    },
    {
        ...
        "options": ["Тип TON-кошелька", "Смарт-контракт ловушка", "Способ заработать на DeFi", "NFT-коллекция"],
        "correct": 1,
        "explanation": "Ханипот — смарт-контракт ловушка: купить токен можно, продать нельзя. Деньги заперты навсегда."
    },
    {
    ...
    {
        "q": "Что такое Jetton на TON?",
        "options": ["Тип NFT", "Токен на TON (аналог ERC-20)", "Название кошелька", "Блокчейн-игра"],
        "correct": 1,
        "explanation": "Jetton — это токены на TON, аналог ERC-20 на Ethereum. Любой проект может выпустить свой Jetton."
    },
    {
        "q": "Какой основной NFT маркетплейс на TON?",
        "options": ["OpenSea", "Rarible", "Getgems.io", "Magic Eden"],
        "correct": 2,
        "explanation": "Getgems.io — главный NFT маркетплейс на TON. Там можно купить, продать и создать NFT."
    },
    {
        "q": "Что такое DEX?",
        "options": ["Децентрализованная биржа", "Тип NFT", "Блокчейн кошелёк", "Токен TON"],
        "correct": 0,
        "explanation": "DEX (Decentralized Exchange) — биржа без посредников. Торгуешь напрямую из кошелька."
    },
    {
        "q": "Какая основная DEX на TON?",
        "options": ["Uniswap", "STON.fi", "PancakeSwap", "dYdX"],
        "correct": 1,
        "explanation": "STON.fi — главная децентрализованная биржа на TON. Там можно свапать Jetton-токены."
    },
    {
        "q": "Что такое TON DNS?",
        "options": ["Антивирус для кошелька", "Домены .ton в блокчейне", "Тип транзакции", "NFT стандарт"],
        "correct": 1,
        "explanation": "TON DNS — система доменных имён на TON. Можно купить домен .ton вместо длинного адреса кошелька."
    },
    {
        "q": "Какой кошелёк встроен прямо в Telegram?",
        "options": ["Tonkeeper", "MyTonWallet", "@wallet", "Tonhub"],
        "correct": 2,
        "explanation": "@wallet — официальный кошелёк прямо в Telegram. Не нужно ничего скачивать!"
    },
    {
        "q": "Что такое rugpull?",
        "options": ["Тип стейкинга", "Команда проекта исчезает с деньгами", "NFT коллекция", "Тип кошелька"],
        "correct": 1,
        "explanation": "Rugpull — когда команда проекта собирает деньги и исчезает. DYOR перед любой инвестицией!"
    },
    {
        "q": "Сколько примерно стоит транзакция в TON?",
        "options": ["$5-10", "$1-2", "Менее $0.01", "$0.5"],
        "correct": 2,
        "explanation": "Транзакции в TON стоят меньше цента. Это одна из главных фишек сети!"
    },
    {
        "q": "Что такое impermanent loss?",
        "options": ["Потеря seed-фразы", "Риск провайдера ликвидности при расхождении цен", "Комиссия за транзакцию", "Тип мошенничества"],
        "correct": 1,
        "explanation": "Impermanent loss — риск для тех кто даёт ликвидность на DEX. Если цены токенов расходятся — получаешь меньше чем просто держал."
    },
    {
        "q": "На каком сайте можно посмотреть любую транзакцию TON?",
        "options": ["etherscan.io", "tonscan.org", "blockchain.com", "bscscan.com"],
        "correct": 1,
        "explanation": "tonscan.org — основной блокчейн эксплорер TON. Любая транзакция, кошелёк, контракт — всё там."
    },
    {
        "q": "Что такое TON Connect?",
        "options": ["Мессенджер на TON", "Протокол подключения кошелька к dApps", "Тип NFT", "Биржа TON"],
        "correct": 1,
        "explanation": "TON Connect — протокол который позволяет подключить кошелёк (например Tonkeeper) к любому dApp на TON."
    },
    {
        "q": "Что означает DYOR в крипто?",
        "options": ["Do Your Own Research", "Don't Yield On Returns", "Decentralized Yield On Rate", "Digital Yield Of Return"],
        "correct": 0,
        "explanation": "DYOR — Do Your Own Research. Всегда изучай проект сам перед тем как вкладывать деньги!"
    },
]
...
async def cmd_help(message: Message):
    await message.answer(
        "🤖 *Что умеет Тоша:*\n\n"
        "🚀 Онбординг — создай TON кошелёк за 5 минут\n"
        "🔍 Анализ кошелька — баланс + AI-инсайт\n"
        "🔎 Расшифровка транзакций — объясню любой хэш\n"
        "💱 Курс TON — USD, EUR, RUB\n"
        "🎯 Квиз — 17 вопросов про TON\n"
        "💡 Факт дня — 40+ фактов об экосистеме\n"
        "🚨 Скам-база — топ схем мошенничества\n"
        "📈 Прогресс — твои шаги онбординга\n\n"
        "Или просто напиши любой вопрос про TON — отвечу 🙌\n\n"
        "*/start* — главное меню\n"
